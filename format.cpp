...
void format(){
	...
	*/
    ...
    FILE *fpRead = fopen("passwd.txt","r");
    char StrLine[1024];
    char p_uid[10],p_gid[10],password[1024];
    int linenum=0;
    char c;
	while((fscanf(fpRead,"%c",&c))!=EOF){
        fgets(StrLine,1024,fpRead); 
        p_uid[0]=c;
        for(i=1;StrLine[i-1]!='-';i++){ //格式为 “用户名-权限-密码 ” 
            p_uid[i]=StrLine[i-1];
        }
        p_uid[i]='\0';
        int temp=i+1;
        for(i=temp;StrLine[i-1]!='-';i++){
            p_gid[i-temp]=StrLine[i-1];
        }
        p_gid[i-temp]='\0';
        int flag=i+1;
        for(i=flag;i<flag+4;i++){
            password[i-flag]=StrLine[i-1];
        }
        password[i-flag]='\0';
        passwd[linenum].p_uid=atoi(p_uid);
        passwd[linenum].p_gid=atoi(p_gid);
        strcpy(passwd[linenum].password,password);
        linenum++;
	}
    fclose(fpRead);
