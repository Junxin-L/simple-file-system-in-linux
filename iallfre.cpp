...
static struct dinode blockBuf[BLOCKSIZ/DINODESIZ];		//存放i节点的临时数组
/*****************************************************
函数：ialloc
...
struct inode * ialloc(){
	...
	if (filsys.s_pinode == NICINOD){
		...
		while (count < NICINOD){		//空闲i节点数组没有装满且磁盘中还有空闲i节点
			if (block_end_flag){
				memcpy(blockBuf, disk+DINODESTART + cur_di*DINODESIZ, BLOCKSIZ);//从i节点去中读一个盘块到临时数组
				block_end_flag = 0;
				i = 0;
			}
			while (blockBuf[IUPDATE].di_mode != DIEMPTY){	//临时数组为空，则读到空闲i节点数组中
				cur_di ++;
				i++;
			}
